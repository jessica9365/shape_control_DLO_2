...
class MySimulator(torch.nn.Module):
    ...
    #     # === TEMPORARY FIX: Use standard forward, disable train_step ===
    ...
    #     return self.output_layer(graph_repr)
    ...
    def forward(self, data):
        ...
        # === LYAPUNOV TRAIN_STEP INTEGRATION ===
        for subnet in self.subnets:
            if subnet.type == 'ee':
                node_out, edge_feature = subnet.train_step(node_feature, node_feature, 
                                                     edge_index, edge_feature, None)
            elif subnet.type == 'en':
                node_out, node_feature = subnet.train_step(node_feature, None)
            elif subnet.type == 'pe':
                node_out, edge_feature, input_to_node = subnet.train_step(node_feature, node_feature, 
                                                                    edge_index, edge_feature, None)
            elif subnet.type == 'pn':
                node_out, node_feature = subnet.train_step(input_to_node, None)
...
class JacobianPredictor(object):
    ...
    def LoadDataForTraining(self, train_dataset=None):
        ...
        self.trainDataLoader = DataLoader(self.trainDataset, batch_size=32, shuffle=True, num_workers=0)
        # self.trainDataLoader = DataLoader(self.trainDataset, batch_size=train_dataset.shape[0], shuffle=True, num_workers=0)

    # ------------------------------------------------------
    def LoadDataForValidation(self, val_dataset=None):
        if val_dataset is None:
            val_dataset = np.load(
                os.path.join(self.dataDir, 'train_data', self.env_dim, 'state_0.npy')
            ).astype(np.float32)[600*0 : 600*2, :]
        self.valDataset = NNDataset(val_dataset.astype(np.float32))
        self.valDataLoader = DataLoader(self.valDataset, batch_size=32, shuffle=False, num_workers=0)
    ...
    def LoadDataForTest(self, test_dataset=None):
        ...
        self.testDataLoader = DataLoader(self.testDataset, batch_size=32, shuffle=False, num_workers=0)
    ...
    def SaveModelWeights(self):
        torch.save(self.model_J.state_dict(), self.nnWeightDir + "model_J_GNN.pth")
    ...
    def Train(self, loadPreModel=False, n_epoch=50, save_model=True, rotation_augmentation=True):
        ...
        log_file = "loss_GNN.txt"
        with open(log_file, "w") as f:
            f.write("Epoch,TrainLoss,ValLoss,ValMAE\n")
        ...
        for epoch in range(0, n_epoch):
            ...
            for batch_idx, (length, state_input, fps_vel, ends_vel) in enumerate(self.trainDataLoader):
                ...
                fps_vel  /= (torch.linalg.norm(fps_vel,  dim=1).unsqueeze(1) + 1e-8)
                ...
                # forward
                # J_pred = self.model_J(graph_batch)
                # J_pred = J_pred.view(-1, args.DLO_num_FPs, 12, 3).transpose(2, 3).reshape(-1, 3*args.DLO_num_FPs, 12)
                # print(J_pred.shape)
                ...
                # forward (Lyapunov happens INSIDE model_J!)
                J_pred = self.model_J(graph_batch)
                J_pred = J_pred.view(-1, args.DLO_num_FPs, 12, 3).transpose(2, 3).reshape(-1, 3*args.DLO_num_FPs, 12)
                J_pred[:, :, [3, 4, 5, 9, 10, 11]] *= length.view(-1, 1, 1)
                ...
                # NO optimizer.step()! Lyapunov handles updates internally
                accumLoss += loss.item()
                numBatch += 1

            # ---- end of one epoch: compute average train loss ----
            epoch_loss = accumLoss / numBatch
            loss_lst.append(epoch_loss)
            print("epoch: ", epoch, " , Loss/train: ", epoch_loss)

            # ---- compute validation loss/MAE at end of epoch ----
            val_loss, val_mae = self.EvaluateOffline()

            # ---- log both train and val ----
            with open(log_file, "a") as f:
                f.write(f"{epoch + 1},{epoch_loss:.6f},{val_loss:.6f},{val_mae:.6f}\n")
            ...
            if val_loss < best_val_loss - 1e-4:   # small margin to avoid noise
                ...
                patience_counter = 0
                if save_model:
                    self.SaveModelWeights()       # save best model so far
                print(f"New best val loss: {best_val_loss:.6f} at epoch {epoch+1}")
            else:
                patience_counter += 1
    ...
    def EvaluateOffline(self):
        ...
        with torch.no_grad():
            for length, state_input, fps_vel, ends_vel in self.valDataLoader:
                # construct graph
                graph_lst = get_graph_batch(state_input, length, numFPs=self.numFPs)
                graph_batch = Batch.from_data_list(graph_lst).to('cuda' if torch.cuda.is_available() else 'cpu')
                ...
                # predict J and velocities
                J_pred = self.model_J(graph_batch)
                J_pred = J_pred.view(-1, self.numFPs, 12, 3).transpose(2, 3).reshape(-1, 3*self.numFPs, 12)
                J_pred[:, :, [3, 4, 5, 9, 10, 11]] *= length.view(-1, 1, 1)

                J_pred_T = J_pred.transpose(1, 2)
                fps_vel_pred = torch.bmm(bmm_ends_vel, J_pred_T)  # [B, 1, 3*numFPs]

                loss = self.criterion(bmm_fps_vel, fps_vel_pred)

                # accumulate loss
                batch_size = fps_vel.shape[0]
                total_loss += loss.item() * batch_size
                # MAE for interpretability
                total_mae  += torch.mean(torch.abs(fps_vel_pred - bmm_fps_vel)).item() * batch_size
                total_samples += batch_size

        avg_val_loss = total_loss / total_samples
        avg_val_mae  = total_mae / total_samples
        print(f"Validation loss: {avg_val_loss:.6f}, Validation MAE: {avg_val_mae:.6f}")
...
if __name__ == '__main__':
    ...
    split_idx = int(0.8 * num_samples)  # 80% train, 20% validation

    train_data = train_dataset[:split_idx, :]
    val_data   = train_dataset[split_idx:, :]

    trainer = JacobianPredictor()
    trainer.LoadDataForTraining(train_data)
    trainer.LoadDataForValidation(val_data)  # new
    trainer.Train(loadPreModel=False, n_epoch=50)
