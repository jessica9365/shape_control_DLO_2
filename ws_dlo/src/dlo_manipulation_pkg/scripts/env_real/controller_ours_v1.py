...
class Controller(object):
    # --------------------------------------------------------------------
    def __init__(self, config_path="C:/Users/91990/Documents/GitHub/FYP_Object_Detection_Model/shape_control_DLO_2/ws_dlo/src/dlo_system_pkg/config/config.json"):
        config = load_config(config_path)

        self.numFPs               = config["DLO_num_FPs"]
        self.env_dim              = config["env_dimension"]
        self.env                  = config["env_sim_or_real"]
        self.bEnableEndRotation   = config["controller_enable_end_rotation"]
        self.b_left_arm           = config["controller_enable_left_arm"]
        self.b_right_arm          = config["controller_enable_right_arm"]
        self.targetFPsIdx         = config["controller_object_fps_idx"]
        self.project_dir          = config["project_dir"]
        self.offline_model_name   = config["controller_offline_model"]
        self.control_law          = config["controller_control_law"]
        self.controlRate          = config["ros_rate_env_rate"]

        # Optional extras from your JSON
        self.tag                  = config["tag"]
        self.onlineUpdateRate     = config["ros_rate_online_update_rate"]
        self.lr                   = config["controller_online_learning_learning_rate"]
        self.weight_ratio         = config["controller_online_learning_weight_ratio"]
        self.region               = config["controller_region"]
        self.Kp_priority          = config["controller_Kp_priority"]

        self.validJacoDim = self.getValidControlInputDim(
            self.env_dim, self.bEnableEndRotation,
    ...
    # --------------------------------------------------------------------
    def generateControlInput(self, state, observed_state=None):
    ...
    def reset(self, state):
        ...
        result_dir = os.path.join(
            self.project_dir, "results", self.env,
            "control", self.control_law, self.env_dim
        ) + "/"
        if not os.path.exists(result_dir):
            os.makedirs(result_dir)

        np.save(result_dir + "state_con_occ_2_" + str(self.case_idx) + ".npy", self.state_save)
        ...
        if self.case_idx == 100:
            print("Finished 100 cases. Stopping.")
            raise SystemExit("Experiment complete.")
