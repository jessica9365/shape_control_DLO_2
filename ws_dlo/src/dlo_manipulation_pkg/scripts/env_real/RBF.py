...
class Net_J(nn.Module):
    ...
    def GetMuAndBetaByKMeans(self, full_data, device):
        ...
        invSigma = 1.0 / np.clip(sigma, 1e-8, None)
        invSigma = np.clip(invSigma, 0, 1)

        self.fc1.centres.data = torch.tensor(kmeans.cluster_centers_).to(device)
        self.fc1.sigmas.data = torch.tensor(invSigma).to(device)
...
class JacobianPredictor(object):
    ...
    numFPs = 10 #rospy.get_param("DLO/num_FPs")
    projectDir = "C:/Users/91990/Documents/GitHub/FYP_Object_Detection_Model/shape_control_DLO_2" #rospy.get_param("project_dir")
    online_learning_rate = 1.0 #rospy.get_param("controller/online_learning/learning_rate")
    lr_task_e = online_learning_rate
    lr_approx_e = online_learning_rate * 10 #rospy.get_param("controller/online_learning/weight_ratio")
    env = "real" #rospy.get_param("env/sim_or_real")
    env_dim = "2D" #rospy.get_param("env/dimension")
    control_rate = 10 #rospy.get_param("ros_rate/env_rate")
    online_update_rate = 50 #rospy.get_param("ros_rate/online_update_rate")
    ...
    def __init__(self, num_hidden_unit=256):
        ...
        print(f"JacobianPredictor using device: {self.device}")
    ...
    def LoadModelWeights(self, file=None):
        if file is not None:
            if os.path.exists(self.nnWeightDir  + "/" + file):
                self.model_J.load_state_dict(torch.load(self.nnWeightDir  + "/" + file, map_location=self.device))
            ...
            if "learning_is_test":
                if os.path.exists(self.nnWeightDir  + "/model_J.pth"):
                    self.model_J.load_state_dict(torch.load(self.nnWeightDir + "/model_J.pth", map_location=self.device))
                    # print('Load previous model.')
                else:
                    print('Warning: no model exists !')
            else:
                if os.path.exists(self.nnWeightDir + offline_model + "/model_J.pth"):
                    self.model_J.load_state_dict(torch.load(self.nnWeightDir + offline_model + "/model_J.pth", map_location=self.device))
                    # print('Load previous model.')
                else:
                    print('Warning: no model exists !')
        ...
        if loadPreModel == False:  # use kmeans to calculate the initial value of mean and sigma of gaussian kernels
            if rotation_augmentation:
                self.model_J.GetMuAndBetaByKMeans(
                        self.relativeStateRepresentationTorch(dataRandomTransform(self.trainDataset.state_input)), self.device)
            else:
                self.model_J.GetMuAndBetaByKMeans(
                        self.relativeStateRepresentationTorch(self.trainDataset.state_input), self.device)
        else:
