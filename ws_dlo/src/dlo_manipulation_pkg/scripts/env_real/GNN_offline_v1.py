...
def load_args_from_file(
        filepath=r"C:/Users/91990/Documents/GitHub/FYP_Object_Detection_Model/shape_control_DLO_2/ws_dlo/src/dlo_system_pkg/config/config.json"
        ):
...
def get_graph_data(state_input, length, numFPs):
    ...
    # else:
    #     # accept either shape (numFPs,) or (numFPs,1)
    #     fp_visible_mask = torch.as_tensor(fp_visible_mask, dtype=torch.float32)
    #     mask_fp = fp_visible_mask.view(numFPs, 1)
    ...
    graph = pyg.data.Data(
        ...
        edge_attr=edge_input.float(),
        all_pos_xyz=all_pos_xyz.float()
    )
    return graph
...
class JacobianPredictor(object):
    ...
    # ------------------------------------------------------
    def Train(self, loadPreModel=False, n_epoch=1, save_model=True, rotation_augmentation=True):
        ...
        for epoch in range(0, n_epoch):
            ...
            for batch_idx, (length, state_input, fps_vel, ends_vel) in enumerate(self.trainDataLoader):
                ...
                graph_lst = get_graph_batch(state_input, length_phys, numFPs=args.DLO_num_FPs)
