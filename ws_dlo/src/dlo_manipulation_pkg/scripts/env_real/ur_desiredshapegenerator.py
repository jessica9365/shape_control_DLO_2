...
# To generate desired shape for dlo manipulation 2D
...
NTU_UR5E_MAIN_IP = '169.254.149.80'
...
# TRANSFORM_MATRIX = {"tf_img_rb1": args_ur.tf_img_rb1_2D,
#                     "tf_rb2_rb1": args_ur.tf_rb2_rb1_2D}
# POSE_START1 = args_ur.pose_start1_2D
# POSE_START2 = args_ur.pose_start2_2D

#CHANGES HERE
TRANSFORM_MATRIX = {"tf_img_rb2_2D_onearm": args_ur.tf_img_rb2_2D_onearm}
POSE_START2 = args_ur.pose_start2_2D



def getState(dlo_length, right_eff, keypoints_vector, prev_keypoints_vector, desired_pose, dim=2):
    state = np.zeros(117)
    """
    ...
    #CHANGES HERE
    right_eff_pose = right_eff.receive.getActualTCPPose()
...
def transformImageToRobot(pixel_coords, tf_matrix, dim=2):
    ...
    for point in pixel_coords:
        ...
        if len(point) == 2:          # (x, y) only — pad z=0
            x, y = point
            z = 0.0
        else:                        # (x, y, z)
            x, y, z = point
        hp = np.array([x, y, z, 1.0])
        new_point = tf_matrix @ hp
        x_new, y_new, z_new, _ = new_point
        ret.append((x_new, y_new, z_new))
    return np.array(ret)
...
def main(): 
    ...
    try:
        ...
        if not (right_eff.is_online):
            print_utils.logerr('Right Robot is not online!')
            return

        # save_dir = controller.project_dir + "/env_dlo/env_" + controller.env_dim + "_Data/StreamingAssets/desired_shape/" + controller.env_dim  + "/"
        save_dir = "C:/Users/91990/Documents/GitHub/FYP_Object_Detection_Model/shape_control_DLO_2/env_dlo/env_2D_Data/StreamingAssets/desired_shape/2D/"
        img_dir = save_dir + "images/"
        if not os.path.exists(save_dir):
            os.makedirs(save_dir)
        if not os.path.exists(img_dir):
            os.makedirs(img_dir)
        savefile = save_dir + "realsim_desired_positions.npy"
        ...
        while True:
            ...
            img_with_fp, keypoints_vector, keypoints_matrix, extra = dlo.detectFeatures(img, keypoints_matrix)
            # CHANGE HERE
            keypoints_vector_in_m = transformImageToRobot(keypoints_vector, TRANSFORM_MATRIX["tf_img_rb2_2D_onearm"], dim=dim) # transform to robot base frame
            ...
            desired_pose_in_m = transformImageToRobot(desired_pose[0], TRANSFORM_MATRIX["tf_img_rb2_2D_onearm"], dim=dim) # transform to robot base frame
