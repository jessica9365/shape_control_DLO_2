...
from GNN_offline_v4 import JacobianPredictor
...
class Controller(object):
    ...
    def __init__(self):
        ...
        self.offline_model_path = 'model_J.pth'
        self.jacobianPredictor.LoadModelWeights(self.offline_model_path) #default weights is /model/refWeights/2D/10*6_backup
    ...
    # --------------------------------------------------------------------
    def generateControlInput(self, state, observed_state, missing_idx=[]):

        self.state_save.append(copy.deepcopy(state))

        #for following calculation, use observed_state
        state=observed_state
        lst = [0, 1, 2, 3, 4, 5, 6, 7]
        obs_idx = [x for x in lst if x not in missing_idx]
        if missing_idx:
            g_node_miss_idx=[[x+1 for x in missing_idx]]
        else:
            g_node_miss_idx = None

        self.targetFPsIdx = [x+1 for x in obs_idx]
        print("Control idx", self.targetFPsIdx)
        ...
        # state_input_torch = self.jacobianPredictor.relativeStateRepresentationTorch(torch.tensor(state_input)).to(self.device)
        ...
        state_input = state_input.double().to(self.device)

        edge_cache={}
        data, edge_cache = get_graph_batch_RBF_comparable_fast(
            state_input, length, numFPs=self.numFPs, missing_lst=g_node_miss_idx,  #missing_lst=g_node_miss_idx
            use_chain_edges=False,  # consider True for speed/stability
        ...
        # J_pred = torch.reshape(torch.reshape(self.jacobianPredictor.model_J(x_node), (-1, self.numFPs, 12, 3)).transpose(2, 3),
        #                         (-1, 3 * self.numFPs, 12))

        J_pred = self.jacobianPredictor.model_J(data) #!!1 no need to reshape, otherwise wrong results

        J_pred[:, :, [3, 4, 5, 9, 10, 11]] *= length.reshape(-1, 1, 1)
        Jacobian = J_pred.cpu().detach().numpy().reshape(3 * self.numFPs, 12)


        # print("NN Graph Jacobian", Jacobian.shape)
        # print("Jacobian magnitude", np.linalg.norm(Jacobian))
    ...
    def reset(self, state):
        ...
        print("saved state")
        print(state[I.state_input_idx][3:27])
        # result_dir = self.project_dir + "results/" + self.env + "/control/" + self.control_law + "/" + self.env_dim + "/"
        result_dir = self.project_dir + "results/" + self.env + "/control/" + self.control_law + "/" 
        ...
        self.jacobianPredictor.LoadModelWeights(self.offline_model_path)
