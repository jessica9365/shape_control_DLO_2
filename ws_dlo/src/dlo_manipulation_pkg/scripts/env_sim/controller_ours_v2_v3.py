...
# from GNN_offline_v3 import JacobianPredictor
...
#     # Coordinate Axis Origin Augmentation: origin at FP1
#     origin = fps_pos[0:1, :]  # [1,3]
...
#     # Node features without mask: 7 dims
#     # [x,y,z, orientation(4)] for ends, [x,y,z,0,0,0,0] for FPs
#     left_end_state  = torch.concat((left_end_pos_rel,  left_end_quat),  dim=-1)  # [1,7]
#     right_end_state = torch.concat((right_end_pos_rel, right_end_quat), dim=-1)  # [1,7]
#     fps_state       = torch.concat((fps_pos_rel, torch.zeros(numFPs, 4)), dim=-1)  # [numFPs,7]

#     # # Build visibility mask: all ones at test time (no occlusions)
#     # mask_left  = torch.ones(1, 1, dtype=torch.float32)
#     # mask_fp    = torch.ones(numFPs, 1, dtype=torch.float32)
#     # mask_right = torch.ones(1, 1, dtype=torch.float32)
#     # mask_channel = torch.cat([mask_left, mask_fp, mask_right], dim=0)  # [numFPs+2,1]

#     # Concatenate nodes and mask → 8-dim node features (matches trained model)
#     node_input = torch.concat((left_end_state, fps_state, right_end_state), dim=0)  # [numFPs+2,7]
#     # node_input = torch.cat([node_input, mask_channel], dim=1)                       # [numFPs+2,8]

#     # Positions for edge construction
#     all_pos_xyz = node_input[:, :3]

#     # Edge index depends on radius
#     edge_index = radius_graph(all_pos_xyz, r=length/3, loop=False)

#     # Edge features: relative distance
...
#     # get a list of graphs (one per sample in batch)
#     return [
#         get_graph_data(state_input_tensor[i:i+1, :], length_tensor[i], numFPs)
#         for i in range(state_input_tensor.shape[0])
#     ]
...
#     # --------------------------------------------------------------------
#     def generateControlInput(self, state, observed_state=None, fp_visible_mask=None):
#         self.state_save.append(copy.deepcopy(state))

#         # Use observed_state for Jacobian/graph if provided
#         state = observed_state if observed_state is not None else state

#         # Mask: if none is given, assume all visible
#         if fp_visible_mask is None:
#             fp_visible_mask = np.ones((self.numFPs,), dtype=np.float32)
#         else:
#             fp_visible_mask = np.asarray(fp_visible_mask, dtype=np.float32).reshape(self.numFPs,)
...
#         self.targetFPsIdx = [1,2,3,4,5,6,7,8] # change based on mask
...
#         # # length_torch = torch.tensor(length).device('cuda' if torch.cuda.is_available() else 'cpu')
#         # # state_input_torch = self.jacobianPredictor.relativeStateRepresentationTorch(torch.tensor(state_input)).device('cuda' if torch.cuda.is_available() else 'cpu')
...
#         # state_input = torch.tensor(state_input, dtype=torch.float32, device=device)

#         # # graph_lst = get_graph_batch(state_input.cpu(), length.cpu(), numFPs=self.numFPs)
#         # graph_lst = get_graph_batch(state_input.cpu(), length.cpu(), numFPs=self.numFPs)

#         # graph_batch = Batch.from_data_list(graph_lst).to(device)

#         # self.jacobianPredictor.model_J = self.jacobianPredictor.model_J.to(device).double()

#         # J_pred = self.jacobianPredictor.model_J(graph_batch)
#         # J_pred = torch.reshape(
#         #     torch.reshape(J_pred, (-1, self.numFPs, 12, 3)).transpose(2, 3),
#         #     (-1, 3 * self.numFPs, 12)
#         # )
...
#         # Use v2 Jacobian predictor with mask-aware 8D node features
#         Jacobian = self.jacobianPredictor.PredictJacobian(state, fp_visible_mask)
#         print("GNN v2 masked Jacobian", Jacobian.shape)
...
# Modified controller: supports v3 (8D masked graphs) while keeping sim_env/model API unchanged
# Changes: Remove local get_graph_data; use v3's version; fix PredictJacobian call
...
# Import v3 model + its graph builders (no local override!)
from GNN_offline_v3 import JacobianPredictor, get_graph_data, get_graph_batch  # v3 versions
from utils.state_index import I  # Keep your indexing
import torch

from torch_geometric.data import Data, Batch
import torch_geometric as pyg
from torch_geometric.nn import radius_graph
from GN_model_DLO import MySubEdge, MySubNode  # Keep if needed by v3
from torch_geometric.nn import global_mean_pool
...
class Controller(object):
    def __init__(self):
        ...
        # Valid control dims unchanged
        self.validJacoDim = self.getValidControlInputDim(self.env_dim, self.bEnableEndRotation, 
                                                         self.b_left_arm, self.b_right_arm)

        self.jacobianPredictor = JacobianPredictor()
        self.jacobianPredictor.LoadModelWeights()  # Loads v3 weights

        self.k = 0
        self.case_idx = 0
        self.state_save = []

    # All other methods unchanged: normalizeTaskError, optimizeControlInput, validStateConstraintMatrix, 
    # reset, getValidControlInputDim (copy from your original)

    def generateControlInput(self, state, observed_state=None, fp_visible_mask=None):
        self.state_save.append(copy.deepcopy(state))

        # Use observed_state for Jacobian/graph if provided (your interpolation!)
        state_for_jacobian = observed_state if observed_state is not None else state

        # Mask: defaults to all visible
        if fp_visible_mask is None:
            fp_visible_mask = np.ones((self.numFPs,), dtype=np.float32)
        else:
            fp_visible_mask = np.asarray(fp_visible_mask, dtype=np.float32).reshape(self.numFPs,)
        ...
        # Task error computation unchanged
        full_task_error = np.zeros((self.numFPs, 3), dtype='float32')
        full_task_error[self.targetFPsIdx, :] = np.array(fpsPositions - desiredPositions).reshape(self.numFPs, 3)[self.targetFPsIdx, :]

        self.targetFPsIdx = [1,2,3,4,5,6,7,8]  # Your tracked FPs
        ...
        # ========== KEY CHANGES: v3-Compatible Jacobian Prediction ==========
        # Extract inputs expected by v3's PredictJacobian or graph builders
        length = state[I.length_idx]  # Scalar
        state_input = state[I.state_input_idx].reshape(1, -1)  # [1, 44] or relative dims
        
        # Convert to tensors for v3 graph building
        device = next(self.jacobianPredictor.model_J.parameters()).device  # Use model's device
        length_t = torch.tensor([length], dtype=torch.float32, device=device)
        state_input_t = torch.tensor(state_input, dtype=torch.float32, device=device)
        mask_t = torch.tensor([fp_visible_mask], dtype=torch.float32, device=device)  # [1, numFPs]

        # Build v3 graph batch (8D nodes, masked edges!)
        graph_lst = get_graph_batch(state_input_t, length_t, mask_t, numFPs=self.numFPs)
        graph_batch = Batch.from_data_list(graph_lst).to(device)
        # graph_batch = graph_batch.float()  # Sequential, not chained # v3 expects double?

        # Predict Jacobian using v3 model directly (bypasses any buggy PredictJacobian)
        self.jacobianPredictor.model_J.eval()
        with torch.no_grad():
            J_pred = self.jacobianPredictor.model_J(graph_batch)  # Raw output (360,)
        
        # Reshape/scale like v3 (adjust indices if your v3 scales differently)
        J_pred = torch.reshape(
            torch.reshape(J_pred, (-1, self.numFPs, 12, 3)).transpose(2, 3),
            (-1, 3 * self.numFPs, 12)
        )
        J_pred[:, :, [3, 4, 5, 9, 10, 11]] *= length_t.reshape(-1, 1, 1)  # Length scaling
        Jacobian = J_pred.cpu().detach().numpy().reshape(3 * self.numFPs, 12)
        
        print("GNN v3 masked Jacobian shape:", Jacobian.shape)
        # ====================================================================

        # Rest unchanged: target_J, fps_vel_ref, optimization...
        target_J = np.zeros((3 * len(self.targetFPsIdx), len(self.validJacoDim)))
        for i, targetIdx in enumerate(self.targetFPsIdx):
            target_J[3*i : 3*i+3, :] = Jacobian[3*targetIdx : 3*targetIdx+3, self.validJacoDim]
    ...
    def generateControlInput(self, state, observed_state=None, fp_visible_mask=None):
        ...
        # After normalized_target_task_error = ...

        # ========== v3 BLOCK (REPLACE OLD PredictJacobian CALL) ==========
        length = state[I.length_idx]
        state_input = state[I.state_input_idx].reshape(1, -1)

        device = next(self.jacobianPredictor.model_J.parameters()).device
        length_t = torch.tensor([length], dtype=torch.float32, device=device)
        state_input_t = torch.tensor(state_input, dtype=torch.float32, device=device)
        mask_t = torch.tensor([fp_visible_mask], dtype=torch.float32, device=device)

        graph_lst = get_graph_batch(state_input_t, length_t, mask_t, numFPs=self.numFPs)
        graph_batch = Batch.from_data_list(graph_lst).to(device)
        # graph_batch = graph_batch.float()  # Sequential, not chained
        ...
        J_pred[:, :, [3, 4, 5, 9, 10, 11]] *= length_t.reshape(-1, 1, 1)
        Jacobian = J_pred.cpu().detach().numpy().reshape(3 * self.numFPs, 12)
