...
class Controller(object):
    # --------------------------------------------------------------------
    def __init__(self):
        self.numFPs = 10 #rospy.get_param("DLO/num_FPs")
        self.env_dim = "2D" #rospy.get_param("env/dimension")
        self.env = "sim" #rospy.get_param("env/sim_or_real")
        self.bEnableEndRotation = True #rospy.get_param("controller/enable_end_rotation")
        self.b_left_arm = True #rospy.get_param("controller/enable_left_arm")
        self.b_right_arm = True #rospy.get_param("controller/enable_right_arm")
        self.targetFPsIdx = [1,2,3,4,5,6,7,8] #rospy.get_param("controller/object_fps_idx")
        self.project_dir = "/home/jessica/shape_control_DLO_2/" #rospy.get_param("project_dir")
        self.offline_model_name = "10_6" #rospy.get_param("controller/offline_model")
        self.control_law = "rbf_c3_error_analysis" #rospy.get_param("controller/control_law")
        self.controlRate = 10 #rospy.get_param("ros_rate/env_rate")
    ...
    def generateControlInput(self, state):
        ...
        #Disabled online learning
        length = state[I.length_idx]
        state_input = state[I.state_input_idx].reshape(1, -1).astype('float32')
        length_torch = torch.tensor(length).to(self.jacobianPredictor.device)
        state_input_torch = self.jacobianPredictor.relativeStateRepresentationTorch(torch.tensor(state_input)).to(self.jacobianPredictor.device)
        with torch.no_grad():
            J_pred = self.jacobianPredictor.model_J(state_input_torch)
            J_pred[:, :, [3, 4, 5, 9, 10, 11]] *= length_torch.reshape(-1, 1, 1)
        Jacobian = J_pred.cpu().numpy().reshape(3 * self.numFPs, 12)
        ...
        #REMOVE LATER
        print(f"target_J.shape: {target_J.shape}")
        print(f"C1.shape: {C1.shape}, all zero? {np.all(C1==0)}")
        print(f"C2.shape: {C2.shape}, all zero? {np.all(C2==0)}")
        print(f"validJacoDim: {self.validJacoDim}")
    ...
    def validStateConstraintMatrix(self, state):
        ...
        if self.env_dim == '3D':
            ...
            C1 = np.zeros((1,6))
            C2 = np.zeros((2, 6))
        # Skip all stretch logic - always return safe zero matrices
        return C1, C2
