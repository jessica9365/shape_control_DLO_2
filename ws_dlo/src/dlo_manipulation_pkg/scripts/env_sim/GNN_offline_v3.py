...
from utils.data_augment import dataRandomTransformWithLength
from utils.state_index import I2 as I
...
class NNDataset(Dataset):
    def __init__(self, arr, numFPs):
        ...
        # mask stored at end of each row (as in your preprocess)
        self.fp_visible_mask = arr[:, -self.numFPs:]
...
# def occlusion_case_random(stateinputtensor, pmask=0.25, numFPs=10, k_range=(1,4)):
#     B = stateinputtensor.shape[0]
#     fpvisiblemask = torch.ones(B, numFPs, dtype=torch.float32)
#     state_out = stateinputtensor.clone()

#     for i in range(B):
#         if np.random.rand() < pmask:
#             k = np.random.randint(k_range[0], k_range[1]+1)
#             hidden_fps = np.random.choice(numFPs, size=k, replace=False)
#             fpvisiblemask[i, hidden_fps] = 0.0 # Just mark as invisible
        
#             #Implementation 2
#             for fp_idx in hidden_fps:
#                 s = 3 * fp_idx
#                 e = 3 * (fp_idx + 1)
#                 state_out[i, s:e] = 0.0  # Zero masked positions!

#     return state_out, fpvisiblemask # Keep original positions!
...
def get_graph_data(state_input, length, fp_visible_mask, numFPs):
    """
    Dynamic Graph Construction for Random Occlusion (Case 1)
    - Keep original positions for all 12 nodes
    - Build edges ONLY between visible nodes (adaptive radius)
    - Fixed 12-node batching preserved
    """
    if state_input.dim() == 1:
        state_input = state_input.unsqueeze(0)  # [44] → [1,44]

    # Extract positions from state_input [1,44] → [12 positions]
    left_end_pos = state_input[:, 3 * numFPs: 3 * numFPs + 3]
    left_end_quat = state_input[:, 3 * numFPs + 3: 3 * numFPs + 7]
    right_end_pos = state_input[:, 3 * numFPs + 7: 3 * numFPs + 10]
    right_end_quat = state_input[:, 3 * numFPs + 10: 3 * numFPs + 14]
    ...
    # # Origin shift (FP0 as reference)
    # origin = fps_pos[0]  # FP1 position
    # fps_pos_rel = fps_pos - origin
    # left_end_pos_rel = left_end_pos - origin
    # right_end_pos_rel = right_end_pos - origin
    
    # Build node features [12, 7] (position + features) 
    left_end_state = torch.concat((left_end_pos, left_end_quat), dim=-1)
    right_end_state = torch.concat((right_end_pos, right_end_quat), dim=-1)
    fps_state = torch.concat((fps_pos, torch.zeros(numFPs, 4)), dim=-1)  # (numFPs,7)

    node_features = torch.cat([left_end_state, 
                              fps_state, 
                              right_end_state], dim=0)  # [12,7]
    
    # Add mask channel [12,1] → ends always visible
    mask_left = torch.ones(1, 1)
    mask_fps = fp_visible_mask.reshape(numFPs, 1)    # [10,1] from occlusion_case_random()
    mask_right = torch.ones(1, 1)
    mask_channel = torch.cat([mask_left, mask_fps, mask_right], dim=0)  # [12,1]
    
    nodeinput = torch.cat([node_features, mask_channel], dim=1)  # [12,8] FINAL
    
    # === DYNAMIC EDGES: VISIBLE NODES ONLY ===
    visible_mask = nodeinput[:, 7] > 0.5  # [12] boolean: ends+visible FPs = True
    # visible_pos = nodeinput[visible_mask, 3:6]  # [N_vis,3] POSITIONS of visible nodes only
    visible_pos = nodeinput[visible_mask, 0:3]
    
    # Adaptive radius: scale by number of visible nodes
    num_visible = visible_mask.sum().item()  # e.g., 9 (2 ends + 7 FPs)
    expected_segment = length / max(num_visible - 1, 1)  # length/(segments)
    dynamic_radius = expected_segment * 1.8  # 80% margin for gaps
    
    # Build edges on VISIBLE positions only
    edge_index_vis = radius_graph(visible_pos, r=dynamic_radius, loop=False)
    
    # Map visible indices back to full 12-node indexing
    visible_indices = torch.where(visible_mask)[0]  # e.g., [0,1,3,4,5,6,7,9,11]
    edge_index = torch.zeros_like(edge_index_vis, dtype=torch.long)
    edge_index[0] = visible_indices[edge_index_vis[0]]  # source nodes
    edge_index[1] = visible_indices[edge_index_vis[1]]  # target nodes

    # Edge features: relative positions
    # src_pos = nodeinput[edge_index[0], 3:6]
    # tgt_pos = nodeinput[edge_index[1], 3:6]
    src_pos = nodeinput[edge_index[0], 0:3]
    tgt_pos = nodeinput[edge_index[1], 0:3]
    edge_attr = src_pos - tgt_pos  # [E,3]


    # # At end of get_graph_data():
    # max_edges = 128  # Safe upper bound
    # edge_index_pad = torch.full((2, max_edges), -1, dtype=torch.long)  # [2,128]
    # edge_index_pad[:, :edge_index.shape[1]] = edge_index                # [2,E_actual]
    # edge_attr_pad = torch.zeros(max_edges, 3)

    # edge_index_pad[:, :edge_index.shape[1]] = edge_index
    # edge_attr_pad[:edge_index.shape[1]] = edge_attr

    batch_tensor = torch.zeros(12, dtype=torch.long)  # Fixed 12 nodes per graph
    return pyg.data.Data(
        x=nodeinput.to(torch.float32),
        edge_index=edge_index,
...
def get_graph_batch(state_input_tensor, length_tensor, fp_visible_mask, numFPs=10):
    ...
    # Line with r_i calculation - fix division:
    if isinstance(length_tensor, torch.Tensor):
...
class MySimulator(torch.nn.Module):
    ...
    #Implementation 2 - remove
    def masked_global_mean_pool(self, node_feat, batch, node_mask, eps=1e-8):
        """
        node_feat: [N, F]
        batch:     [N] graph id per node
        node_mask: [N] 1.0 visible, 0.0 hidden
        """
        node_mask = node_mask.view(-1, 1)  # [N,1]
        node_feat_masked = node_feat * node_mask

        sum_feat = pyg.nn.global_add_pool(node_feat_masked, batch)  # [B,F]
        sum_mask = pyg.nn.global_add_pool(node_mask, batch)         # [B,1]

        return sum_feat / (sum_mask + eps)
    ...
    def forward(self, data):
        ...
        # Mask-aware pooling: mask is last feature channel (index 7)
        node_mask = data.x[:, 7]  # [N], 1 visible, 0 hidden
        graph_repr = self.masked_global_mean_pool(node_feature, batch, node_mask)
...
class JacobianPredictor(object):
    ...
    # env_dim = '2D_mask_1'  # hard coded
    control_rate = args.ros_rate_env_rate
    online_update_rate = args.ros_rate_online_update_rate
    weight_folder = args.weight_folder
    # weight_folder = 'gnn_v3' # hard coded
    loss_log_folder = args.loss_log_folder
    loss_log_filename = args.loss_log_filename
    # loss_log_filename = 'gnn_v3_mask_1'  # hard coded
    ...
    def LoadDataForTraining(self, train_dataset=None):
        ...
        if train_dataset is None:
            train_dataset = np.load(self.dataDir + 'train_data/' + self.env_dim + '/state_0.npy').astype(np.float32)[
                600 * 2: 600 * 10, :]
        self.trainDataset = NNDataset(train_dataset.astype(np.float32), numFPs=self.numFPs)
    ...
    def LoadDataForTest(self, test_dataset=None):
        ...
        if test_dataset is None:
            test_dataset = np.load(self.dataDir + 'train_data/' + self.env_dim + '/state_0.npy').astype(np.float32)[
                600 * 0: 600 * 2, :]
        self.testDataset = NNDataset(test_dataset.astype(np.float32), numFPs=self.numFPs)
    ...
    def SaveModelWeights(self):
        model_dir = os.path.join(self.nnWeightDir, self.weight_folder)
        os.makedirs(model_dir, exist_ok=True)  # Create directory!
        
        model_path = os.path.join(model_dir, "model_J_v3.pth")
        torch.save(self.model_J.state_dict(), model_path)
    ...
    def Train(self, loadPreModel=False, n_epoch=50, save_model=True, rotation_augmentation=True):
        ...
        log_dir = self.loss_log_folder  # Directory name
        os.makedirs(log_dir, exist_ok=True)  # Create if doesn't exist
        log_file = os.path.join(log_dir, "loss_GNN_case1_v1.txt")           
        ...
        for epoch in range(0, n_epoch):
            accumLoss = 0.0
            numBatch = 0
            for batch_idx, (length, state_input, fps_vel, ends_vel, fp_visible_mask) in enumerate(self.trainDataLoader):
                ...
                fp_visible_mask = fp_visible_mask.to('cuda' if torch.cuda.is_available() else 'cpu')  # Ensure mask is on GPU if available
                 # ---- NEW: randomly hide contiguous and non-contiguous FP blocks in some samples ----
                # state_input, fp_visible_mask = occlusion_case_random(state_input, pmask=0.25, numFPs=args.DLO_num_FPs)
    ...
    def TestAndSaveResults(self):
        ...
        accumLoss = 0.0
        numBatch = 0
        for batch_idx, (length, state_input, fps_vel, ends_vel, fp_visible_mask) in enumerate(self.testDataLoader):
            # state_input = self.relativeStateRepresentationTorch(state_input)

            # construct graph

            # Option A: no masking in test
            fp_visible_mask = torch.ones(state_input.shape[0], args.DLO_num_FPs)
            graph_lst = get_graph_batch(state_input, length, fp_visible_mask, numFPs=args.DLO_num_FPs)
